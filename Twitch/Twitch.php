...
class Twitch
{
	...
	function __construct(array $options = [])
	{
		...
		if ($this->verbose) $this->log('[CONSTRUCT]');
	...
	{
		if ($this->verbose) $this->log('[CLOSE]');
		if ($closeLoop) {
			$this->closing = true;
			if ($this->verbose) $this->log('[LOOP->STOP]');
			$this->loop->stop();
		}
	}
	...
	public function sendMessage(string $data, ?string $channel = null): void
	{
		...
		$this->log('[REPLY] #' . ($channel ?? $this->reallastchannel ?? current($this->channels)) . ' - ' . $data);
	...
	{
		if ($this->verbose) $this->log('[VERBOSE] [JOIN CHANNEL] `' . $string . '`');
		if($string){
	...
	*/
	...
	{
		if ($this->verbose) $this->log('[VERBOSE] [LEAVE CHANNEL] `' . $string . '`');
	...
	*/ 
	protected function connect(): void
	{
		...
		$port = '6667';
		if ($this->verbose) echo "[CONNECT] $url:$port";
		$this->emit("CONNECT" ,  array("Connect $url:$port")  );
		...
		if(!$twitch->connection){
			...
			$this->connector->connect("$url:$port")->then(
				function (ConnectionInterface $connection) use ($twitch) {
					...
					$twitch->log('[CONNECTED]');
				},
				function (Exception $exception) {
					$twitch->log('[ERROR] ' . $exception->getMessage());
				}
			);
		} else $twitch->log('[SYMANTICS ERROR] A connection already exists!');
	}
	protected function initIRC(ConnectionInterface $connection): void
	{
		...
		if ($this->verbose) $this->log('[INIT IRC]');
	}

	protected function pingPong(string $data, ConnectionInterface $connection): void
	{
	if ($this->debug) $this->log("[DEBUG] [" . date('h:i:s') . "] PING :tmi.twitch.tv");
		$connection->write("PONG :tmi.twitch.tv\n");
	if ($this->debug) $this->log("[DEBUG] [" . date('h:i:s') . "] PONG :tmi.twitch.tv");
	}
	
	protected function process(string $data, ConnectionInterface $connection): void
	{
		if ($this->debug) $this->log("[DEBUG] [DATA] " . $data);
	...
	protected function parseMessage(string $data): ?string
	{
		...
		if ($this->verbose) $this->log('[PRIVMSG] (#' . $this->reallastchannel . ') ' . $this->reallastuser . ': ' . $messageContents);
		...
		if ($commandsymbol) {
			...
			if ($this->verbose) $this->log("[COMMAND] `$command`"); 
			...
			if ( in_array($this->lastuser, $this->whitelist) || ($this->lastuser == $this->nick) ) {
				if (in_array($command, $this->restricted_functions)) {
					if ($this->verbose) $this->log('[RESTRICTED FUNCTION]');
					$response = $this->commands->handle($command, $dataArr);
				}
			}
			...
			if ($this->lastuser == $this->nick) {
				if (in_array($command, $this->private_functions)) {
					if ($this->verbose) $this->log('[PRIVATE FUNCTION]');
					$response = $this->commands->handle($command, $dataArr);
				}
			}
	...
	public function discordRelay($payload): void
	{
		if ($this->discord_output){
			if ($this->verbose) $this->log('[DISCORD CHAT RELAY]');
			if(
