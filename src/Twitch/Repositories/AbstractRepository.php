...
/**
 * Repositories provide a way to store and update parts on the Twitch server.
 *
 * @since 4.0.0
 *
...
abstract class AbstractRepository extends Collection
{
    ...
    /**
     * Endpoints for interacting with the Twitch servers.
     *
     * @var array Endpoints.
     */
    protected $endpoints = [];
     ...
     */
    public function __construct(Twitch $twitch, array $vars = [])
    {
        $this->http = $twitch->getHttpClient();
        $this->factory = $twitch->getFactory();
        $this->vars = $vars;
        if ($cacheConfig = $twitch->getCacheConfig(static::class)) {
            $this->cache = new CacheWrapper($twitch, $cacheConfig, $this->items, $this->class, $this->vars);
        } else {
            $this->cache = new LegacyCacheWrapper($twitch, $this->items, $this->class);
        }
    ...
    /**
     * Attempts to save a part to the Twitch servers.
     *
    ...
    /**
     * Attempts to delete a part on the Twitch servers.
     *
    ...
    /**
     * Gets a part from the repository or Twitch servers.
     *
